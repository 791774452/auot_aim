...
def run(weights=ROOT / 'yolov5s.pt',  # 权重路径
    ...
    pid_x = PID(0.5, 0.2, 0.00, setpoint=0,output_limits=(-100,100))
    pid_y = PID(0.5, 0.0, 0.00, setpoint=0,output_limits=(-100,100))
    pid = (pid_x, pid_y)
    while True:
        ...
        for i, det in enumerate(pred):  # 每张图片
            ...
            if len(aims):
                ...
                for i, det in enumerate(aims):
                    ...
                    location_y = round(top_left[1] + (bottom_right[1] - top_left[1]) / 4 + top_y)
                    # 移到鼠标
                    if MOUSE_STATE:
                        # 调用pid自瞄
                        auto_aim(location_x, location_y, int(width), i, pid)
...
def auto_aim(x, y, width, i, pid):
    ...
    if result < width * 2:
        ...
        pid_aim((x, y), pid,i)
        # coordinate = win32api.GetCursorPos()
        # pydirectinput.moveTo(coordinate[0],coordinate[1]+ 4)
        # win32api.SetCursorPos((coordinate[0], coordinate[1] + 10))
        ...
        # 方形范围
        # j=1
        # if x - width*j < coordinate[0] < x + width*j and y - width*j < coordinate[1] < y + width*j :
        #    if abs(x - coordinate[0]) > width/8 and abs(y - coordinate[1]) > width/8:
        #        win32api.SetCursorPos((x, y))
        # print("类型", i, "定位坐标", x, ",", y)
    # coordinate = win32api.GetCursorPos()
    # win32api.SetCursorPos((coordinate[0], coordinate[1] + 10))
...
# PID瞄准
def pid_aim(aim_x_y, pid,i):
    print("PID启动", aim_x_y)
    x_y = win32api.GetCursorPos()
    # print("当前坐标", x_y[0], ",", x_y[1])
    pid_x = pid[0]
    pid_y = pid[1]
    x = pid_x(x_y[0] - aim_x_y[0])
    y = pid_y(x_y[1] - aim_x_y[1])
    # print(x, y)
    to_xy = round(x_y[0] + x), round((x_y[1]) + y)
    print("类型",i,x_y, "需要位置", aim_x_y, "--( ", x, y, " )--PID移到到--->", to_xy)
    win32api.mouse_event(win32con.MOUSEEVENTF_MOVE, round(x), round(y), 0, 0)
    # win32api.SetCursorPos(x_y)
    # pydirectinput.moveTo(x_y[0],x_y[1])
# win32api.mouse_event(win32con.MOUSEEVENTF_MOVE, 0, 8, 0, 0)


def parse_opt():
    parser = argparse.ArgumentParser()
    # parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'MODEL/yolov5s.onnx',help='模型路径,支持pt,onnx,tensorrt')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'data/CF-600img.pt',
...
def on_click(x, y, button, pressed):
    global MOUSE_STATE
    print('{0} at {1}'.format('按下Pressed' if pressed else '松开Released', (x, y)))
    mutex.acquire()
    MOUSE_STATE = pressed
    mutex.release()
